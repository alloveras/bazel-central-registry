...
Three top-level aliases expose the public libraries:

| Target          | Library                                   |
|-----------------|-------------------------------------------|
| `@icu//:icuuc`  | `libicuuc` — Unicode core                 |
| `@icu//:icui18n`| `libicui18n` — Collation, formatting, ... |
| `@icu//:icuio`  | `libicuio` — stdio-compatible Unicode I/O |

Each library is compiled as a single monolithic `cc_library` rather than
mirroring the fine-grained upstream targets.
...
`icu4c/source/tools/bootstrap.bzl`. `bootstrap_cc_binary` is a drop-in for
